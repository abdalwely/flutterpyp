...
class _FuturisticProfileScreenState extends ConsumerState<FuturisticProfileScreen>
          ...
          return Stack(
            children: [
              ...
              currentUserAsync.when(
                data: (user) {
                  if (user == null) {
                    return _buildErrorContent(customColors);
                  }
                  return _buildProfileContent(context, user, customColors);
                },
                loading: () => _buildLoadingContent(customColors),
                error: (error, _) => _buildErrorContent(customColors),
              ),
            ],
          );
        },
  ...
  Widget _buildProfileContent(BuildContext context, user, Map<String, Color> customColors) {
    return SafeArea(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              _buildProfileHeader(user, customColors),

              const SizedBox(height: 40),

              // Stats Cards
              _buildStatsCards(user, customColors),

              const SizedBox(height: 30),

              // Profile Options
              _buildProfileOptions(user, customColors),

              const SizedBox(height: 30),

              // Settings Section
              _buildSettingsSection(customColors),

              const SizedBox(height: 40),

              // Logout Button
              _buildLogoutButton(customColors),
  ...
  Widget _buildProfileHeader(user, Map<String, Color> customColors) {
    return Transform.translate(
  ...
  Widget _buildStatsCards(user, Map<String, Color> customColors) {
    return Transform.translate(
  ...
  Widget _buildStatCard(String title, String value, IconData icon, Color color, Map<String, Color> customColors) {
    return Container(
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(20),
  ...
  Widget _buildProfileOptions(user, Map<String, Color> customColors) {
    return Transform.translate(
  ...
  Widget _buildSettingsSection(Map<String, Color> customColors) {
    return Transform.translate(
      offset: Offset(0, 10 * _slideAnimation.value),
      child: Opacity(
        opacity: _fadeInAnimation.value,
        child: Column(
          ...
          children: [
            ...
            _buildOptionItem(
              ...
              subtitle: 'تخصيص ألوان وثيم التطبيق',
  ...
  Widget _buildLogoutButton(Map<String, Color> customColors) {
    return Transform.translate(
      offset: Offset(0, 10 * _slideAnimation.value),
      child: Opacity(
        opacity: _fadeInAnimation.value,
  ...
  Widget _buildLoadingContent(Map<String, Color> customColors) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
  ...
  Widget _buildErrorContent(Map<String, Color> customColors) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
  ...
  // فتح صفحة إعدادات الثيم
  void _openThemeSettings() {
    Navigator.of(context).pushNamed(ThemeSettingsScreen.routeName);
  }
        ...
        decoration: BoxDecoration(
          ...
          gradient: LinearGradient(
            colors: [
              ...
              ref.read(themeProvider).accentColor,
            ],
          ),
        ),
        child: Icon(
  ...
  Future<void> _updateProfileImage() async {
    ...
    try {
      ...
      _showErrorMessage('خطأ في تحديث الصورة: ${e.toString()}');
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _updateUserName(String userId, String newName) async {
    ...
    try {
      ...
      _showErrorMessage('خطأ في تحديث الاسم: ${e.toString()}');
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
...
class ProfileBackgroundPainter extends CustomPainter {
  ...
  void _drawSparkleEffect(Canvas canvas, Size size, Paint paint) {
    paint.shader = null;
    paint.color = primaryColor.withOpacity(0.6 * sparkleValue);
...
class ProfileParticlesPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ...
      ..color = primaryColor.withOpacity(0.3 * animationValue);
